...
class Exchange(Events):
    ...
    def add_to_order_book(self, stock_code, buy_sell, volume, price, user_reference):
        ...
        then build the pricelevels for each stock & buySell and add to order book
        """
    ...
    def has_best_price_changed(self, order):
        ...
        we shouldn't compare new price level to when there's no sell price level as 0 will always be the min value
        """
    ...
    def get_price_levels(self, stock_code, buy_sell):
        ...
        When getting best price level, a SortedDict will give better performance than a regular Dict
        """
...
print(f'current order id is {exchange.current_order_id}')
